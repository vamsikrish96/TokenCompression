        ...
        for token in json_tokens:
            ...
            if preserve:
                for i in range(token.start, min(token.end, len(mask))):
                    mask[i] = True
            elif token.token_type == JSONTokenType.STRING_VALUE:
                # A compressible value still keeps its own quotes. Contiguous
                # compressible tokens merge into a single span, so without
                # this the span runs from inside the string, through the
                # closing quote, and on into the following whitespace --
                # truncating it then drops the quote and pulls a real newline
                # into the string, which is invalid JSON (RFC 8259 section 7).
                # Pinning the delimiters bounds every span to the value.
                mask[token.start] = True
                if token.end - 1 < len(mask):
                    mask[token.end - 1] = True
