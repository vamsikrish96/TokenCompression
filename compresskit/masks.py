...
preserved) vs "compressible" (can be reduced or dropped).

This separates the concerns of:
1. Structure detection (handlers) - What tokens are navigational?
2. Content reduction (reducers) - What tokens can be shrunk or removed?
...
class StructureMask:
    ...
    (it's structural/navigational), False means "compressible" (a reducer can
...
def apply_mask_to_text(
    ...
    Args:
        ...
        compress_fn: Function to compress text (e.g., ``reducers.reduce_text``).
